...
This project may only be successfully run from a Linux Machine because of the dependency and use of
`all-repos` and `ansible-core`.
...
.step 0: setup like any normal python repository
[subs="quotes"]
----
...
2. Add your note (e.g. `ansible-roles mylaptop`)
   and select a preferred expiration date
3. Select the required permissions (most-often just `public_repo`)
...
=== Clone (execute this command before any maintenance command!)
